...
function ServerHostPlayers($sid, $type="servers", $obId="", $tplsid="", $open="", $inHome=false, $trunchostname=48)
{
    ...
    if ($type == "servers") {
        if (!empty($info['HostName'])) {
            ...
            $objResponse->addAssign("os_$sid", "innerHTML", "<i class='$os fa-2x' style='color: #ff3c1a;'></i>");
            if ($info['Secure']) {
                $objResponse->addAssign("vac_$sid", "innerHTML", "<i class='fas fa-shield-alt fa-2x' style='color: #067edb;'></i>");
            }
